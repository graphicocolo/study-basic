...
#### プリミティブ値とオブジェクトで挙動が違う

配列のスロット（インデックス）と、その中身を分けて考えるとわかりやすい。

**中身がプリミティブ値（文字列・数値）の場合**

```js
const origin = ["cat", "dog", "mouse"];
const copy = [...origin];

origin[0] = "bird"; // origin の 0番スロットを差し替え

console.log(origin[0]); // "bird"
console.log(copy[0]);   // "cat" ← 変わらない
```

スプレッドで新しい配列スロットが作られ、値そのものがコピーされる。
`origin[0] = "bird"` は origin の 0番スロットの指す先を変えるだけで、copy の 0番スロットとは無関係。

**中身がオブジェクト（参照型）の場合**

```js
const origin = [{ name: "cat" }, { name: "dog" }];
const copy = [...origin];

// origin[0] と copy[0] は同じオブジェクトを指している
origin[0].name = "bird"; // 実体を直接書き換え

console.log(origin[0].name); // "bird"
console.log(copy[0].name);   // "bird" ← 同じ実体なので変わってしまう
```

スロットに入っているのはオブジェクトへの参照（住所）なので、両方が同じ実体を指す。
実体を直接書き換えると両方に影響する。

ただし、スロット自体を差し替えた場合はコピーに影響しない：

```js
origin[0] = { name: "fish" }; // origin の 0番スロットを差し替え

console.log(origin[0].name); // "fish"
console.log(copy[0].name);   // "bird" ← copy[0] は元のオブジェクトを指したまま
```

| 中身の型 | スプレッドの挙動 | 元を変更すると？ |
|---|---|---|
| 文字列・数値（プリミティブ） | 値そのものがコピーされる | コピーは変わらない |
| オブジェクト・配列（参照型） | 参照（住所）がコピーされる | **同じ実体を指すので変わる** |

「参照が共有される」のはネストされたオブジェクトに対して起きる話で、プリミティブ値は最初から値がコピーされるため影響しない。
